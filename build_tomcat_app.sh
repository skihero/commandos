...
#  
usage(){
	echo "Usage: $0 appname tomcat_withapp version release"
	exit 1
}
# 
# Usage 
#  ./build_tomcat_app.sh  tomcat-admin-1 tomcat-admin-1 1 0  
# 
#
# Exit code status 
#  1  usaga
...
Packager: infra 
%description 
${APP_NAME} tomacat with application rpm

%install 
mkdir -p \${RPM_BUILD_ROOT}/usr/local/release3/${APP_NAME}
cp -r ../SOURCES/${TOM_DIR}/*  \${RPM_BUILD_ROOT}/usr/local/release3/${APP_NAME}

%files 
/usr/local/release3/${APP_NAME} 
# END_OF_SPEC
EOF
